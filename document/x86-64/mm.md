...
| 虚拟地址范围 | 描述 | 访问权限 | 用途 |
| ------------ | ---- | -------- | ---- |
| 0xFFFF800000000000 - 0xFFFFFFFF80000000 | 内核代码段 | 内核态可执行 | 内核代码、只读数据 |
| 0xFFFFFFFF80000000 - 0xFFFFFFFFA0000000 | 内核数据段 | 内核态可读写 | 内核全局变量、静态数据 |
| 0xFFFFFFFFA0000000 - 0xFFFFFFFFC0000000 | 内核栈 | 内核态可读写 | 内核线程栈空间 |
| 0xFFFFFFFFC0000000 - 0xFFFFFFFFE0000000 | 内核堆 | 内核态可读写 | 内核动态内存分配 |
| 0xFFFFFFFFE0000000 - 0xFFFFFFFFF0000000 | 页表区 | 内核态可读写 | 四级页表存储 |
| 0xFFFFFFFFF0000000 - 0xFFFFFFFFFFFFFFFF | 硬件映射区 | 内核态可访问 | 硬件设备内存映射 |
| 0x0000800000000000 - 0x00007FFFFFFFFFFF | 保留区域 | 不可访问 | 防止地址溢出 |
