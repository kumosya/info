...
class Request {
 public:
    Request(Bio *b)
        : sector(b->sector), count(b->count), buf(b->buf), rw(b->rw),
          next(nullptr), bio_list(b) {}
    ~Request() {
        Bio *b = bio_list;
        while (b) {
            Bio *next = b->next;
...
class RequestQueue {
 public:
    RequestQueue() : head(nullptr), tail(nullptr), request_fn(nullptr),
                     max_sectors(0), max_hw_sectors(0), plugged(false), count(0) {}
    ~RequestQueue() {
        while (!empty()) {
            Request *req = pop();
            delete req;
...
class BlockDevice {
 public:
    BlockDevice(const char *dev_name, DeviceType dev_type)
        : queue(new RequestQueue()), capacity(0), sector_size(512),
          type(dev_type), partitions(nullptr), part_count(0), next(nullptr) {
    ...
    virtual int Write(std::uint64_t sector, std::uint32_t count, const void *buf) = 0;
...
class IDEBlockDevice : public BlockDevice {
 public:
    IDEBlockDevice(const char *dev_name, std::uint16_t io_base, std::uint8_t drive)
        : BlockDevice(dev_name, DEVICE_TYPE_IDE), io_base_(io_base), drive_(drive) {
    ...
    int Write(std::uint64_t sector, std::uint32_t count, const void *buf) override;
