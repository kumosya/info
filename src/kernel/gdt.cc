...
void SetEntry(int index, std::uint64_t base, std::uint64_t limit,
    ...
    std::uint32_t low =
        (std::uint32_t)((limit & 0xFFFF) | ((base & 0xFFFF) << 16));
    std::uint32_t high =
        (std::uint32_t)(((base >> 16) & 0xFF) | ((std::uint32_t)access << 8) |
                        (((limit >> 16) & 0xF) << 16) |
                        ((std::uint32_t)(gran & 0xF) << 20) |
                        (((base >> 24) & 0xFF) << 24));

    gdt_table[index] = ((std::uint64_t)high << 32) | low;
    // tty::printk("GDT Entry %d set: base=0x%lx, limit=0x%lx, access=0x%x,
    // gran=0x%x\n",
...
void SetTss(int index, std::uint64_t tss_base, std::uint8_t access) {
    ...
    std::uint32_t low =
        (std::uint32_t)((tss_limit & 0xFFFF) | ((tss_base & 0xFFFF) << 16));
    std::uint32_t high =
        (std::uint32_t)(((tss_base >> 16) & 0xFF) |
                        ((std::uint32_t)access << 8) |
...
void Init() {
    ...
    SetEntry(1, 0, 0, GDT_PRESENT | GDT_DPL_RING0 | GDT_TYPE_CODE | GDT_TYPE_RW,
             0x2);
    // Data segment descriptor (64-bit, 0x10)
    // Access: 0x92 = present, ring 0, data, writable
    SetEntry(2, 0, 0, GDT_PRESENT | GDT_DPL_RING0 | GDT_TYPE_DATA | GDT_TYPE_RW,
             0x0);
