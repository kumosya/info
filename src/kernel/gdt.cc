...
void Init() {
    ...
    // Code segment descriptor (32-bit, 0x18)
    SetEntry(3, 0, 0xfffff, 0x9a, 0xc);
    // Data segment descriptor (32-bit, 0x20)
    SetEntry(4, 0, 0xfffff, 0x92, 0);
    ...
    // User code segment descriptor (32-bit, 0x48)
    SetEntry(7, 0, 0, 0, 0);
    // User data segment descriptor (32-bit, 0x50)
    SetEntry(8, 0, 0, 0, 0);

    void *stack = mm::page::Alloc(STACK_SIZE);
    tss.rsp0 = tss.rsp1 = tss.rsp2 = reinterpret_cast<std::uint64_t>(stack) + STACK_SIZE;
