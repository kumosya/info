...
MountFs *Ext2Mount(FileSystem *fs, const char *device, const char *path, 
    ...
    if (ext2::ReadSuperBlock(dev, &mount_data->super, mount_data->ext2_lba) != 0) {
...
File *Ext2Open(MountFs *mount, const char *path, std::uint32_t flags) {
    ...
    int ret_path = ext2::LookupPath(mount_data->device, &mount_data->super, lookup_path, 
                          &inode_num, mount_data->ext2_lba);
    if (ret_path != 0) {
        if (flags & 0x02) {
            int ret = ext2::CreateFile(mount_data->device, &mount_data->super, 
                                      lookup_path, EXT2_S_IFREG | 0644, mount_data->ext2_lba);
            if (ret <= 0) {
                tty::printk("EXT2: Failed to create file '%s', error code: -%d\n", lookup_path, -ret);
                return nullptr;
            }
            inode_num = ret;
        } else {
...
ssize_t Ext2Read(File *file, void *buf, std::size_t count, 
    ...
    return ext2::ReadFile(mount_data->device, &mount_data->super, 
                          &file_data->inode, buf, count, offset, mount_data->ext2_lba);
}
...
ssize_t Ext2Write(File *file, const void *buf, std::size_t count, 
    ...
    if (ret > 0) {
        ext2::WriteInode(mount_data->device, &mount_data->super, 
                         file_data->inode_num, &file_data->inode, mount_data->ext2_lba);
    }
...
int Ext2Mkdir(MountFs *mount, const char *path) {
    ...
    int ret = ext2::CreateFile(mount_data->device, &mount_data->super, 
                              path, EXT2_S_IFDIR | 0755, mount_data->ext2_lba);
    if (ret <= 0) {
        return -3;
    }
...
int Ext2Rmdir(MountFs *mount, const char *path) {
    ...
    return ext2::DeleteFile(mount_data->device, &mount_data->super, lookup_path, mount_data->ext2_lba);
}
...
DirEntry *Ext2Readdir(MountFs *mount, const char *path, std::uint32_t index) {
    ...
    return ext2::Readdir(mount_data->device, &mount_data->super, inode_num, index, mount_data->ext2_lba);
}
...
void RegisterFileSystems() {
    FileSystem *ext2_fs = reinterpret_cast<FileSystem *>(mm::page::Alloc(sizeof(FileSystem)));
    if (ext2_fs) {
