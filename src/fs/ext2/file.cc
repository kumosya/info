...
ssize_t ReadFile(block::BlockDevice *dev, Ext2SuperBlock *sb, Ext2Inode *inode, 
    ...
    while (bytes_read < bytes_to_read) {
        ...
        std::uint32_t block_num = GetBlockNum(inode, block_index, sb, dev, ext2_lba);
        if (block_num == 0) {
            break;
        }
...
ssize_t WriteFile(block::BlockDevice *dev, Ext2SuperBlock *sb, Ext2Inode *inode, 
    ...
    while (bytes_written < bytes_to_write) {
        ...
        memcpy(block_buf + block_offset, in_buf + bytes_written, bytes_in_block);
...
int CreateFile(block::BlockDevice *dev, Ext2SuperBlock *sb, const char *path, 
    ...
    //tty::printk("EXT2: CreateFile - parent_inode.i_mode=0x%x, EXT2_S_IFDIR=0x%x\n", 
    ...
    if (AddEntry(dev, sb, &parent_inode, new_inode_num, filename, file_type, ext2_lba) != 0) {
        FreeInode(dev, sb, new_inode_num, ext2_lba);
        return -11;
    }
