...
int ReadBlock(block::BlockDevice *dev, Ext2SuperBlock *sb,
    ...
    std::uint64_t sector =
        ext2_lba + (std::uint64_t)block_num * sectors_per_block;
...
std::uint32_t AllocBlock(block::BlockDevice *dev, Ext2SuperBlock *sb,
    ...
    std::uint32_t group_count =
        (sb->s_blocks_count + blocks_per_group - 1) / blocks_per_group;
    ...
    for (std::uint32_t group = 0; group < group_count; group++) {
        ...
        for (; block_in_group < blocks_per_group; block_in_group++) {
            ...
            if (!(bitmap[byte] & (1 << bit))) {
                ...
                if (WriteBlock(dev, sb, gd.bg_block_bitmap, bitmap, ext2_lba) !=
                    0) {
                    mm::page::Free(bitmap);
                    return 0;
                }
                ...
                std::uint8_t *zero_buf =
                    (std::uint8_t *)mm::page::Alloc(block_size);
                if (zero_buf) {
...
std::uint32_t AllocInode(block::BlockDevice *dev, Ext2SuperBlock *sb,
    ...
    std::uint32_t group_count =
        (sb->s_inodes_count + inodes_per_group - 1) / inodes_per_group;
    ...
    for (std::uint32_t group = 0; group < group_count; group++) {
        ...
        for (std::uint32_t i = 0; i < inodes_per_group; i++) {
            ...
            if (!(bitmap[byte] & (1 << bit))) {
                ...
                if (WriteBlock(dev, sb, gd.bg_inode_bitmap, bitmap, ext2_lba) !=
                    0) {
                    mm::page::Free(bitmap);
                    return 0;
                }
